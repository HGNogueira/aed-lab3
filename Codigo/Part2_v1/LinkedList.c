 ...
 *    
 *    Description:
 *      Evaluates item1 < item2 according to getAttribute func
 *      
 *    Arguments:
 *      both items to be evaluated:
 *        Item item1, Item item2
 *      pointer to getAttribute function:
 *        int (getAttribute *)(Item entry)
 *
 *    Returns 0 if item1 >= item2, else returns 1
 */

int less(Item item1, Item item2, int (* getAttribute)(Item entry)) {
    return getAttribute(item1) < getAttribute(item2);
}
                           ...
                           int * err)
{
  ...
  *err = 0;
  ...
  if(less(item, first->this, getAttribute)) {
    node->next = first;
    return node;
  }

  for(aux = first; aux->next != NULL; aux = aux->next) {
      if(less(item, aux->next->this, getAttribute)) {
          node->next = aux->next;
          aux->next = node;
          return first;
      }
