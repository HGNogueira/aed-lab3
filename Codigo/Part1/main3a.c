...
int main(int argc, char **argv)
{
  ...
  int n_ops = 0;                    /* number of computational operations */
  ...
  if ((criterio == ALFABETICA) && (sentido == ASCENDENTE)) 
    sort((Item) tabela, 0, n_palavras - 1, alphaLess, &n_ops);

  if ((criterio == ALFABETICA) && (sentido == DESCENDENTE)) 
    sort((Item) tabela, 0, n_palavras - 1, alphaMore, &n_ops);

  if ((criterio == OCORRENCIAS) && (sentido == ASCENDENTE))
    sort((Item) tabela, 0, n_palavras - 1, ocorrLess, &n_ops);

  if ((criterio == OCORRENCIAS) && (sentido == DESCENDENTE))
    sort((Item) tabela, 0, n_palavras - 1, ocorrMore, &n_ops);

  if ((criterio == COMPRIMENTO) && (sentido == ASCENDENTE))
    sort((Item) tabela, 0, n_palavras - 1, strlenLess, &n_ops);

  if ((criterio == COMPRIMENTO) && (sentido == DESCENDENTE))
    sort((Item) tabela, 0, n_palavras - 1, strlenMore, &n_ops);
